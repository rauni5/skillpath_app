        ...
        actions: [
          IconButton(
            icon: const CircleAvatar(
              ...
              child: Icon(Icons.person, size: 16, color: AppColors.indigo),
            ),
        ...
        child: Builder(builder: (context) {
          switch (dashboard.state) {
            ...
            case DashboardLoadState.error:
              return ErrorView(message: dashboard.errorMessage ?? 'Something went wrong.', onRetry: _load);
            case DashboardLoadState.loaded:
              ...
              return ListView(
                ...
                children: [
                  ...
                  const SectionHeader(label: 'NEXT UP', icon: Icons.bolt_outlined),
                  const SizedBox(height: 8),
                  if (data.nextSkillsToLearn.isEmpty)
                    const Text('You\'re all caught up on your roadmap 🎉',
                        style: TextStyle(fontSize: 12.5, color: AppColors.textMuted))
                  else
                    Wrap(
                      spacing: 6,
                      runSpacing: 6,
                      children: data.nextSkillsToLearn.map((s) => SkillChipWidget(skill: s)).toList(),
                    ),

                  const SizedBox(height: 18),

                  // Active projects
                  const SectionHeader(label: 'ACTIVE PROJECTS', icon: Icons.groups_outlined),
                  const SizedBox(height: 8),
                  if (data.activeProjects.isEmpty)
                    Container(
                      ...
                      child: const Row(
                        children: [
                          Icon(Icons.explore_outlined, size: 18, color: AppColors.textMuted),
                          SizedBox(width: 10),
                          Expanded(
                            child: Text('No active projects yet — browse open projects to join one.',
                                style: TextStyle(fontSize: 12.5, color: AppColors.textMuted)),
                          ),
                        ],
                      ),
                    ...
                    ...data.activeProjects.map((p) => Padding(
