...
class _SkillsScreenState extends State<SkillsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final loading = skills.catalogState == SkillsLoadState.loading || skills.userSkillsState == SkillsLoadState.loading;
    final hasError = skills.catalogState == SkillsLoadState.error || skills.userSkillsState == SkillsLoadState.error;
      ...
      body: AnimatedSwitcher(
        ...
        child: loading && skills.catalog.isEmpty
            ...
            : hasError && skills.catalog.isEmpty
                ? ErrorView(key: const ValueKey('error'), message: skills.errorMessage ?? 'Something went wrong.', onRetry: _load)
                : RefreshIndicator(
                      ...
                      children: [
                        SectionHeader(
                          ...
                          trailing: skills.userSkills.isEmpty
                              ? null
                              : Text('${skills.userSkills.length}',
                                  style: const TextStyle(fontSize: 11.5, color: AppColors.indigo, fontWeight: FontWeight.w700)),
                        ),
                          ...
                          Container(
                            ...
                            child: const Row(
                              children: [
                                ...
                                SizedBox(width: 10),
                                Expanded(
                                  child: Text('No skills added yet — tap any skill below to add it.',
                                ...
                                .map((s) => SizedBox(
                                      ...
                                      child: OwnedSkillTile(
                                        ...
                                        onRemove: () {
                                          ...
                                          final userId = _userId;
                                          if (userId != null) skills.removeSkill(userId, s.id);
                                        },
                                      ),
                                    ))
                                .toList(),
                        ...
                        const SectionHeader(label: 'ADD SKILLS', icon: Icons.add_circle_outline),
                        const SizedBox(height: 10),
                        TextField(
                        ...
                        const SizedBox(height: 10),
                        CategoryFilterRow(selected: skills.categoryFilter, onSelect: skills.setCategoryFilter),
                          ...
                          Padding(
                            ...
                            child: Column(
                              children: const [
                                ...
                                SizedBox(height: 8),
                                Text('No matching skills.', style: TextStyle(fontSize: 12.5, color: AppColors.textMuted)),
                              ],
                            ),
                          )
                        else
                          ...skills.filteredCatalog.map((s) => CatalogSkillRow(
                                skill: s,
                                isPending: skills.pendingSkillIds.contains(s.id),
                                onAdd: (proficiency) {
                                  HapticFeedback.selectionClick();
                                  final userId = _userId;
                                  if (userId != null) skills.addSkill(userId, s, proficiency);
                                },
                              )),
                      ],
                    ),
