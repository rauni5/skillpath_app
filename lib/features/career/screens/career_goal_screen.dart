...
class _CareerGoalScreenState extends State<CareerGoalScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final loading = career.gapState == CareerLoadState.loading || career.rolesState == CareerLoadState.loading;
    final hasError = career.gapState == CareerLoadState.error && career.rolesState == CareerLoadState.error;
    ...
    return Scaffold(
      ...
      body: AnimatedSwitcher(
        ...
        child: loading && career.roles.isEmpty
            ...
            : hasError
                ? ErrorView(key: const ValueKey('error'), message: career.errorMessage ?? 'Something went wrong.', onRetry: _load)
                : RefreshIndicator(
                      ...
                      children: [
                        if (!_changingGoal) ...[
                          ...
                          FilledButton.icon(
                            ...
                            icon: Icon(career.gap.hasGoalSet ? Icons.swap_horiz : Icons.flag_outlined, size: 18),
                            label: Text(career.gap.hasGoalSet ? 'Change career goal' : 'Choose a career goal'),
                            style: FilledButton.styleFrom(backgroundColor: AppColors.indigo, foregroundColor: Colors.white),
                          ),
                        ] else ...[
                          const SectionHeader(label: 'CHOOSE A ROLE', icon: Icons.explore_outlined),
                          const SizedBox(height: 10),
                          for (final role in career.roles)
                            RoleCard(
                              role: role,
                              isSelected: career.gap.careerRoleName == role.name,
                              onTap: career.isSubmitting ? () {} : () => _selectRole(role.id),
                            ),
